...
static int ICACHE_FLASH_ATTR http_chunked_decode(const char *chunked,
						 char *decode)
{
	int i = 0, j = 0;
	int decode_size = 0;
...
static void ICACHE_FLASH_ATTR http_receive_callback(void *arg, char *buf,
						    unsigned short len)
{
...
static void ICACHE_FLASH_ATTR http_send_callback(void *arg)
{
	...
	if (req->post_data == NULL) {
		...
		if (req->secure)
			espconn_secure_send(conn, (uint8_t *) req->post_data,
					    strlen(req->post_data));
		else
			espconn_send(conn, (uint8_t *) req->post_data,
				     strlen(req->post_data));
...
static void ICACHE_FLASH_ATTR http_connect_callback(void *arg)
{
	...
	char buf[69 + strlen(req->method) + strlen(req->path) +
		 strlen(req->hostname) + strlen(req->headers) +
		 strlen(post_headers)];
	int len = os_sprintf(buf,
			     ...
			     "Connection: close\r\n"
			     "User-Agent: ESP8266\r\n" "%s" "%s" "\r\n",
			     req->method, req->path, req->hostname, req->port,
		...
		if (req->buffer == NULL) {
			...
			if ((os_strncmp
				...
				if (os_strstr
					...
					int body_size =
					    req->buffer_size - (body -
								req->buffer);
					char chunked_decode_buffer[body_size];
					os_memset(chunked_decode_buffer, 0,
						  body_size);
					/* Chuncked data */
					http_chunked_decode(body,
							    chunked_decode_buffer);
					os_memcpy(body, chunked_decode_buffer,
						  body_size);
				}
			}
		}
...
static void ICACHE_FLASH_ATTR http_dns_callback(const char *hostname,
						ip_addr_t * addr, void *arg)
{
	...
	if (addr == NULL) {
		...
		struct espconn *conn =
		    (struct espconn *)os_malloc(sizeof(struct espconn));
		...
		os_timer_disarm(&(req->timeout_timer));
		os_timer_setfn(&(req->timeout_timer),
			       (os_timer_func_t *) http_timeout_callback, conn);
				    ...
				    http_callback_t callback_handle)
{
	...
	http_raw_request(hostname, port, secure, method, path, headers,
			 post_data, callback_handle);
}
...
void ICACHE_FLASH_ATTR http_callback_example(char *response, int http_status,
					     char *full_response)
{
