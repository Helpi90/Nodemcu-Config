...
iram LOCAL void pu_key_intr_handler(struct keys_param *keys);
iram LOCAL void pd_key_intr_handler(struct keys_param *keys);
 ...
 * Returns      : none
*******************************************************************************/
void ICACHE_FLASH_ATTR pu_key_init(struct keys_param *keys)
{
    uint8 i;

    ETS_GPIO_INTR_ATTACH(pu_key_intr_handler, keys);
 ...
 * Returns      : none
*******************************************************************************/
LOCAL void ICACHE_FLASH_ATTR pu_key_5s_cb(struct single_key_param *single_key)
{
 ...
 * Returns      : none
*******************************************************************************/
LOCAL void ICACHE_FLASH_ATTR pu_key_50ms_cb(struct single_key_param *single_key)
{
    ...
    for (i = 0; i < keys->key_num; i++) {
        if (gpio_status & BIT(keys->single_key[i]->gpio_id)) {
            ...
            if (keys->single_key[i]->key_level == 1) {
                ...
                os_timer_setfn(&keys->single_key[i]->key_5s, (os_timer_func_t *)pu_key_5s_cb, keys->single_key[i]);
                ...
                os_timer_setfn(&keys->single_key[i]->key_50ms, (os_timer_func_t *)pu_key_50ms_cb, keys->single_key[i]);
...
/////////////////////////////////////////////////////////////////
void ICACHE_FLASH_ATTR pd_key_init(struct keys_param *keys)
{
    uint8 i;

    ETS_GPIO_INTR_ATTACH(pd_key_intr_handler, keys);
 ...
 * Returns      : none
*******************************************************************************/
LOCAL void ICACHE_FLASH_ATTR pd_key_5s_cb(struct single_key_param *single_key)
{
    os_timer_disarm(&single_key->key_5s);
 ...
 * Returns      : none
*******************************************************************************/
LOCAL void ICACHE_FLASH_ATTR pd_key_50ms_cb(struct single_key_param *single_key)
{
    os_timer_disarm(&single_key->key_50ms);
...
iram LOCAL void pd_key_intr_handler(struct keys_param *keys)
{
    ...
    for (i = 0; i < keys->key_num; i++) {
        if (gpio_status & BIT(keys->single_key[i]->gpio_id)) {
            ...
            if (keys->single_key[i]->key_level == 0) {
                // 5s, restart & enter softap mode
                os_timer_disarm(&keys->single_key[i]->key_5s);
                os_timer_setfn(&keys->single_key[i]->key_5s, (os_timer_func_t *)pd_key_5s_cb, keys->single_key[i]);
                os_timer_arm(&keys->single_key[i]->key_5s, 5000, 0);
                keys->single_key[i]->key_level = 1;
                gpio_pin_intr_state_set(GPIO_ID_PIN(keys->single_key[i]->gpio_id), GPIO_PIN_INTR_NEGEDGE);
            } else {
                // 50ms, check if this is a real key up
                os_timer_disarm(&keys->single_key[i]->key_50ms);
                os_timer_setfn(&keys->single_key[i]->key_50ms, (os_timer_func_t *)pd_key_50ms_cb, keys->single_key[i]);
                os_timer_arm(&keys->single_key[i]->key_50ms, 50, 0);
            }
        }
    }
