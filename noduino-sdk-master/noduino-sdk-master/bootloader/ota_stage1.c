...
void main()
{
	...
	if (!fwInfo.useSecondImageInfo)
		spiRead((offset + 1) * PAGESIZE, &imageInfo, sizeof(imageInfo));
	else
		spiRead((offset + 2) * PAGESIZE, &imageInfo, sizeof(imageInfo));

	ets_memcpy(0x40108000, 0x3ffe8010, 0x304);
	ets_printf("jump to run user");
	if (imageInfo.activeImageNumber == 255)
		imageInfo.activeImageNumber = 0;
