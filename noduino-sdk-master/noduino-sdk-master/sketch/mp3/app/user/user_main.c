...
const unsigned int ICACHE_RODATA_ATTR fakePwm[] = {
	0x00000010, 0x00000410, 0x00400410, 0x00400C10, 0x00500C10, 0x00D00C10,
	    0x20D00C10, 0x21D00C10,
	0x21D80C10, 0xA1D80C10, 0xA1D80D10, 0xA1D80D30, 0xA1DC0D30, 0xA1DC8D30,
	    0xB1DC8D30, 0xB9DC8D30,
	0xB9FC8D30, 0xBDFC8D30, 0xBDFE8D30, 0xBDFE8D32, 0xBDFE8D33, 0xBDFECD33,
	    0xFDFECD33, 0xFDFECD73,
...
int sampToI2sDeltaSigma(short s)
{
	...
	for (x = 0; x < 32; x++) {
		...
		if (outReg > 0)
			...
			w += 32767;	//Difference 1
		w += i1v;
		i1v = w;	//Integrator 1
		if (outReg > 0)
			w -= 32767;
		else
			w += 32767;	//Difference 2
		w += i2v;
		i2v = w;	//Integrator 2
		outReg = w;	//register
		if (w > 0)
...
int ICACHE_FLASH_ATTR recalcAddDelSamp(int oldVal)
{
	...
	if (spiRamFifoLen() < 5 * 1024) {
		...
		ret =
		    oldVal +
		    ((minFifoFill - tgt) * ADD_DEL_BUFFPERSAMP_NOSPIRAM);
		prevUdr += udr;
		minFifoFill = 9999;
	} else {
...
static enum mad_flow ICACHE_FLASH_ATTR error(void *data,
...
void ICACHE_FLASH_ATTR tskmad(void *pvParameters)
{
	...
	if (stream == NULL) {
		printf("MAD: malloc(stream) failed\n");
		return;
	}
	if (synth == NULL) {
		printf("MAD: malloc(synth) failed\n");
		return;
	}
	if (frame == NULL) {
		printf("MAD: malloc(frame) failed\n");
		return;
	}
	//Initialize I2S
	i2sInit();
...
int ICACHE_FLASH_ATTR openConn(const char *streamHost, const char *streamPath)
{
	while (1) {
		...
		printf("Connecting to server %s...\n",
		       ipaddr_ntoa((const ip_addr_t *)&remote_ip.sin_addr.
				   s_addr));
		if (connect
		    (sock, (struct sockaddr *)(&remote_ip),
...
void ICACHE_FLASH_ATTR tskreader(void *pvParameters)
{
	...
	while (1) {
		...
		do {
			...
			if (t == 0)
				printf
				    ("Buffer fill %d, DMA underrun ct %d, buff underrun ct %d\n",
				     spiRamFifoFill(), (int)i2sGetUnderrunCnt(),
				     (int)bufUnderrunCt);
		} while (n > 0);
		close(fd);
...
void ICACHE_FLASH_ATTR tskconnect(void *pvParameters)
{
	...
	if (xTaskCreate(tskreader, "tskreader", 230, NULL, PRIO_READER, NULL) !=
	    pdPASS)
