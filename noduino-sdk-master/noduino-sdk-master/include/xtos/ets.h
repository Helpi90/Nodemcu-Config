 ...
 * characters into to. If the length of from is more than size, then
 * strncpy copies just the first size characters. Note that in this case
 * there is no null terminator written into to. If the length of from
 * is less than size, then strncpy copies all of from, followed by
 * enough null characters to add up to size characters in all. This
 * behavior is rarely useful, but it is specified by the ISO C standard.
 * The behavior of strncpy is undefined if the strings overlap.
 * Using strncpy as opposed to strcpy is a way to avoid bugs relating
 * to writing past the end of the allocated space for to. However, it
 * can also make your program much slower in one common case: copying a
 * string which is probably small into a potentially large buffer. In
 * this case, size may be large, and when it is, strncpy will
 ...
 * written in into account. To get that one
 * has to use strcoll.
*/
 ...
 * needle rather than just a single character. It returns a pointer into the
 * string haystack that is the first character of the substring, or a null
 * pointer if no match was found. If needle is an empty string, the function
 * returns haystack
*/
 ...
 * against the size bytes of memory beginning at a2. The value returned
 * has the same sign as the difference between the first differing pair
 ...
 * is undefined if the two arrays to and from overlap; use memmove instead
 * if overlapping is possible. The value returned by memcpy is the value of to
*/
...
/*
 * This function copies the value of c (converted to an unsigned char)
 ...
 * replacing any format specifier in the same way as printf does, but using
 * the elements in the variable argument list identified by arg instead of
 * additional function arguments. Internally, the function retrieves arguments
 * from the list identified by arg as if va_arg was used on it, and thus the
 * state of arg is likely altered by the call. In any case, arg should have
 * been initialized by va_start at some point before the call, and it is
 * expected to be released by va_end at some point after the call.
 */
...
/*
 * { putc1_external=r3; if(r3!=NULL) ets_install_putc2(r3);
 * else ets_install_putc2(_putc2_def); putc2_external=a4; }
*/
